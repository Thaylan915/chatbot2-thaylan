...
export default function ConhecimentoArea() {
  ...
  async function handleReindexarBase() {
    if (
      !confirm(
        "Reindexar todos os documentos? Isso pode demorar alguns minutos.",
      )
    )
      return;
    setReindexando(true);
  ...
  const ultima = documentos.reduce((max, d) => {
    ...
    return !max || new Date(d.indexado_em) > new Date(max)
      ? d.indexado_em
      : max;
  }, null);
  const resumo = {
  ...
  return (
    <div className="conhecArea">
      ...
      <div className="filtrosBar">
        ...
        <div className="filtroAcoes">
          <button className="btnFiltrar" onClick={handleFiltrar}>
            Filtrar
          </button>

          {(filtroTipo || filtroDataInicio || filtroDataFim) && (
            <button className="btnLimpar" onClick={handleLimparFiltros}>
              Limpar filtros
            </button>
          )}
        </div>
